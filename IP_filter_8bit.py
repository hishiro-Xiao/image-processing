...
class Filter8bit:
    @staticmethod
    def filter8bit(filename, filter):
        # 计算程序运行时间
        ...
        for index in range(img_size):
            ...
            ].reshape((filter_size ** 2, 1))
            pixel_new = int(sum(filter * pixels))
            ...
            out.putpixel((out_x, out_y), pixel_new)

        out.save(file[:-4] + '_filter8bit' + file[-4:])
